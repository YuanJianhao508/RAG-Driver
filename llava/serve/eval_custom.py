...
def main(args):
    ...
    # json_file = "./video_process/conv_new_icl/conversation_bddx_eval.json"
    json_file = args.input
    # out_json_paths = [f"./video_process/conv_new_base_vicuna_iclzs/BDDX_Test_pred_{cap}.json" for cap in ['action','justification']]
    os.makedirs(args.output, exist_ok=True)
    out_json_paths = [f"{args.output}/BDDX_Test_pred_{cap}.json" for cap in ['action','justification']]
    ...
    for item in tqdm(data):
        q1, q2 = item["conversations"][0]["value"], item["conversations"][2]["value"]
        conv.messages.clear()
        ...
        inst_answers = []
        for qid, question in enumerate([q1,q2]):
            # print(question)
            inp = question
